...
class G2SBatchData():
    def __init__(
            ...
            data_indice: torch.Tensor = None,
            # multi_tgts: torch.Tensor = None,
            pres: torch.Tensor = None,
            posts: torch.Tensor = None
    ):
        ...
        # ++++++++++++++++++++++++
        self.data_indice = data_indice
        # self.multi_tgts = multi_tgts
        # self.N_tgt = N_tgt
        # self.N_tgt_lens = N_tgt_lens
        self.pres = pres
        self.posts = posts
        # ++++++++++++++++++++++++
    ...
    def to(self, device):
        ...
        # ++++++++++++++++++++++++
        self.data_indice = self.data_indice.to(device)
        # self.multi_tgts = self.multi_tgts.to(device)
        # self.N_tgt = self.N_tgt.to(device)
        # self.N_tgt_lens = self.N_tgt_lens.to(device)
        self.pres = self.pres.to(device)
        self.posts = self.posts.to(device)
        # ++++++++++++++++++++++++
    ...
    def pin_memory(self):
        ...
        # ++++++++++++++++++++++++
        self.data_indice = self.data_indice.pin_memory()
        # self.multi_tgts = self.multi_tgts.pin_memory()
        # self.N_tgt = self.N_tgt.pin_memory()
        # self.N_tgt_lens = self.N_tgt_lens.pin_memory()
        self.pres = self.pres.pin_memory()
        self.posts = self.posts.pin_memory()
        # ++++++++++++++++++++++++
...
class G2SDataMemory(DataLoad):
    def __init__(
        ...
        # assert '[SH+]' not in self.vocab_data['token_idx'], print(self.vocab_data['token_idx'].items())

        # origin_data_dir = f'{origin_dir}/{dataset_name}'
        # data_file = os.path.join(origin_data_dir, f"raw_{self.mode}.csv")
        # data_df = pd.read_csv(data_file)
        # ptr = []
        # last_sp = None
        # for i, rxn in enumerate(data_df['reactants>reagents>production']):
        #     prod = rxn.split(">>")[1]
        #     if prod != last_sp:
        #         ptr.append(i)
        #         last_sp = prod
        # ptr.append(data_df.shape[0])
        # self.ptr = torch.tensor(ptr, dtype=torch.long)

        self.subs_data = self.data[self.mode]['subs']
        self.subs_seq_len = [len(seq) for seq in self.subs_data]
        self.prod_data = self.data[self.mode]['prod']
        self.prod_seq_len = [len(seq) for seq in self.prod_data]
        self.reaction_type = self.data[self.mode]['reaction_type']
        ...
        ptr = remove_dup(self.data[self.mode]['ptr'])
        self.ptr = np.append(ptr, self.data[self.mode]['ptr'].shape[0])

        # 先 copy 所有 tgt，用于记录一对多，然后挑 batch_idx 去训练
        # subs_tot_seq_len = [len(seq) for seq in self.subs_seq]
        # subs_seq_maxlen = max(subs_tot_seq_len)
        # self.subs_seq = sequence_pad(
        #     seq=self.subs_seq,
        #     seq_len=subs_tot_seq_len,
        #     max_len=subs_seq_maxlen,
        #     pad=self.token_idx['<PAD>']
        # )
        # prod_tot_seq_len = [len(seq) for seq in self.prod_seq]
        # prod_seq_maxlen = max(prod_tot_seq_len)
        # self.prod_seq = sequence_pad(
        #     seq=self.prod_seq,
        #     seq_len=prod_tot_seq_len,
        #     max_len=prod_seq_maxlen,
        #     pad=self.token_idx['<PAD>']
        # )
    ...
    def get_batch_data(self, batch_idx: np.ndarray, task='prod2subs') -> G2SBatchData:
        ...
        if task == 'prod2subs':
            src_graph = copy.deepcopy(self.prod_data['graph'][batch_idx])
            tgt_seq = copy.deepcopy(self.subs_data['token'][batch_idx])
            bi_label = np.array([0] * batch_idx.shape[0])
        elif task == 'subs2prod':
            src_graph = copy.deepcopy(self.subs_data['graph'][batch_idx])
            tgt_seq = copy.deepcopy(self.prod_data['token'][batch_idx])
        ...
        # for diverse prediction
        ptr = self.to_tensor(self.ptr, torch.long)
        pres, posts = [], []
        for id in batch_idx:
            post_id = (ptr > id).nonzero()[0][0]
            pre = ptr[post_id - 1]
            post = ptr[post_id]
            pres.append(pre)
            posts.append(post)
        ...
        pres = torch.tensor(pres, dtype=torch.long)
        posts = torch.tensor(posts, dtype=torch.long)
        # max_len = min(12, (posts - pres).max())
        # TODO: max(tgt_lengths) may not be the truth

        # tgt_ids = self.to_tensor(tgt_tot_seq, dtype=torch.long)
        ...
        return_data = G2SBatchData(
            ...
            task=task,
            # ++++++++++++++++++++++++
            data_indice=torch.as_tensor(batch_idx, dtype=torch.long),
            # multi_tgts=ts,
            pres=pres,
            posts=posts,
            # N_tgt=N_tgt,
            # N_tgt_lens=N_tgt_lens
            # ++++++++++++++++++++++++
        )
    ...
    def get_batch_data_with_subs(self, batch_idx: np.ndarray) -> G2SBatchData:
        ...
        """pres, posts = [], []
        for id in batch_idx:
            post_id = (self.ptr > id).nonzero()[0]
            pre = self.ptr[post_id - 1]
            post = self.ptr[post_id]
            pres.append(pre)
            posts.append(post)
        pres = torch.cat(pres, dim=-1)
        posts = torch.cat(posts, dim=-1)
        max_len = min(8, (posts - pres).max())
        # TODO: max(tgt_lengths) may not be the truth
...
class G2SFullDataset(Dataset):
    ...
    def get_batch(self):
        ...
        if self.token_limit == 0:  # running batch according to fixed batch size
            ...
            # 对原数据分块，构成多个batch，self.batch_order记录的是块的起始下标
...
class S2SDataMemory(DataLoad):
    ...
    def get_batch_data(self, batch_idx: np.ndarray, task='prod2subs') -> S2SBatchData:
        ...
        if task == 'prod2subs':
            src_seq = copy.deepcopy(self.prod_data['token'][batch_idx])
            tgt_seq = copy.deepcopy(self.subs_data['token'][batch_idx])
            bi_label = np.array([0] * batch_idx.shape[0])
        elif task == 'subs2prod':
            src_seq = copy.deepcopy(self.subs_data['token'][batch_idx])
            tgt_seq = copy.deepcopy(self.prod_data['token'][batch_idx])
            bi_label = np.array([1] * batch_idx.shape[0])

        src_seq_len = [len(seq) for seq in src_seq]
        tgt_seq_len = [len(seq) for seq in tgt_seq]
        src_seq_maxlen, tgt_seq_maxlen = max(src_seq_len), max(tgt_seq_len)
    ...
    def get_batch_data_with_subs(self, batch_idx: np.ndarray) -> S2SBatchData:
        ...
        subs_seq, prod_seq = (copy.deepcopy(self.subs_data['token'][batch_idx]),
                              copy.deepcopy(self.prod_data['token'][batch_idx]))
        subs_seq_len, prod_seq_len = ([len(seq) for seq in subs_seq],
                                      [len(seq) for seq in prod_seq])
        seq_maxlen = max(max(subs_seq_len), max(prod_seq_len))
        subs_seq = sequence_pad(
        ...
        src_seq_len = subs_seq_len + prod_seq_len
        bi_label = np.array([0] * len(subs_seq_len) + [1] * len(prod_seq_len))
        # _ for _ in range(n)
        pair_label = np.array([_ for _ in range(len(subs_seq_len))] * 2)
