...
def add_common_args(parser, mode):
    ...
    group.add_argument("--dataset_name", help="Data name", type=str,
                       choices=["uspto_50k", "debug_uspto_50k", "uspto_mit", "uspto_full", "uspto_diverse", "pistachio", "extension", "uspto_full_ms"],
    ...
    group.add_argument("--split_type", help="how to tokenize SMILES", type=str, choices=['token', 'char'], default='token')
...
def get_module_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--model_type', help='which model to use? BiG2S, + HCVAE?', type=str,
                       choices=['BiG2S', 'BiG2S_HCVAE_RXN', 'S2S', 'S2S_HCVAE_RXN', 'G2_LW_CVAT', 'S2S_LW_CVAT', 'G2_LW_RXN_CVAT', 'S2S_LW_RXN_CVAT'], default='BiG2S_HCVAE')
    group.add_argument('--temp', help='temperature in gumbel softmax sampling', type=float, default=1.0)
    group.add_argument('--loss_type', help='which loss to use? focal, ce, mixed-ce?', type=str,
                       choices=['focal', 'CE', 'Mixed-CE'], default='CE')
    group.add_argument('--cvae_type', help='hierarchical modeling latent space?', type=str,
                       choices=['hcvae', 'dcvae'], default='hcvae')
    group.add_argument('--cvae_layers', help='number of layers in cvat', type=int)
    group.add_argument('--lat_fuse_type', help='how to fuse latent variables', type=str, choices=['concat','low_rank'], default='low_rank')
...
def get_train_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--eval_epoch', help='in these epoch, module will evaling but not saving', nargs='*', type=int, default=[0])
    ...
    group.add_argument('--anneal_kl', action='store_true', help="whether anneal kl weight during training")
    group.add_argument('--cycle_iters', help='iters for each annealing', type=int, default=2)

    group.add_argument('--lat_z_size', help='size of layer-wise latent variable', type=int, default=64)
    group.add_argument('--latent_lmf_rank', help='rank of tensor product of latent variable and value in attention',
                       type=int, default=64)
    group.add_argument('--lat_disc_size', help='size of discrete valued latent variable', type=int, default=90)
    group.add_argument('--use_supcon', help='whether use supervised contrastive loss', action='store_true')
    parser.add_argument('--contrast_temp', type=float, default=0.2, help='temperature for contrastive loss')
...
def get_beam_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--beam_strategy', help='use huggingface of OpenNMT model to running beam search, huggingface may slower 3 or more times than OpenNMT,\
                                                        but OpenNMT has some bugs which cause different batch size generate different result, and also a lower accuracy.',
                       type=str, choices=['huggingface', 'OpenNMT'], default='huggingface')
...
def get_eval_args(parser: argparse.ArgumentParser):
    ...
    group.add_argument('--ckpt_list', help='use a loop to eval the checkpoint inside this list', nargs='+', type=str, default=['swa9'])             # 'swa9'
...
def post_setting_args(parser: argparse.ArgumentParser):
    ...
    if args.request == 'preprocess':
        ...
        args.save_name = args.dataset_name + '-' + args.split_type + '-' + args.model_type + '-' + args.beam_module + '-' + args.train_task + '-' + args.loss_type
        if args.model_type in ['BiG2S_HCVAE_RXN', 'S2S_HCVAE_RXN', 'G2_LW_CVAT', 'S2S_LW_CVAT', 'G2_LW_RXN_CVAT']:
            args.save_name += '-' + args.cvae_type + '-lat_sz' + str(args.lat_disc_size) + f'-lwz-{args.lat_z_size}' + f'layers-{args.cvae_layers}'
        if args.model_type == 'G2_LW_CVAT':
            args.save_name += f'fuse_tp-{args.lat_fuse_type}'
        args.save_name += f'-epoch{args.epochs}'
        if args.use_reaction_type:
            args.save_name += f'-typed'
        if args.use_supcon:
            args.save_name += f'-supcon'
        if args.load_ckpt:
            args.save_name += f'-conti'
        ...
        if args.dataset_name in ['debug_uspto_50k', '50k_class', 'uspto_diverse', 'uspto_50k']:
            args.epochs = 800
        elif args.dataset_name == 'uspto_full_ms':
            args.epochs = 400       # 400 for b812
        else:
            args.epochs = 200       # 200 for uspto_full

        # 149 in default
        args.save_epoch = [_ for _ in range(100, args.epochs, 5)] + [args.epochs] \
                            if args.dataset_name in ['uspto_50k', '50k_class', 'uspto_diverse', 'uspto_full_ms'] \
                            else [_ for _ in range(19, args.epochs, 1)] + [args.epochs]

        # # 128 in default
        if args.dataset_name in ['uspto_50k', '50k_class', 'uspto_diverse', 'debug_uspto_50k', 'uspto_full_ms']:
            if args.train_task == 'prod2subs':
                args.use_subs = False
                args.batch_size = 128
        ...
        args.token_limit = 12000 if args.dataset_name in ['uspto_full', 'pistachio', 'pistachio_class'] else 0
        args.memory_clear_count = 1 if args.dataset_name in ['uspto_50k', '50k_class', 'uspto_diverse', "debug_uspto_50k", "uspto_full_ms"] else 4
        args.lr = 1 if args.dataset_name in ['uspto_50k', '50k_class', 'uspto_diverse', "debug_uspto_50k", "uspto_full_ms"] else 1.25
        args.dropout = 0.3 if args.dataset_name in ['uspto_50k', '50k_class', 'uspto_diverse', "debug_uspto_50k", "uspto_full_ms"] else 0.1
        ...
        args.save_name = args.dataset_name + '-' + args.split_type + '-' + args.model_type + '-' + args.beam_module + '-' + args.train_task + '-' + args.loss_type
        if args.model_type in ['BiG2S_HCVAE_RXN', 'S2S_HCVAE_RXN', 'G2_LW_CVAT', 'S2S_LW_CVAT', 'G2_LW_RXN_CVAT']:
            args.save_name += '-' + args.cvae_type + '-lat_sz' + str(
                args.lat_disc_size) + f'-lwz-{args.lat_z_size}' + f'layers-{args.cvae_layers}'
        if args.model_type == 'G2_LW_CVAT':
            args.save_name += f'fuse_tp-{args.lat_fuse_type}'
        args.save_name += f'-epoch{args.epochs}'
        if args.use_reaction_type:
        ...
        #128
        args.token_limit = 12000 if args.dataset_name in ['uspto_full'] else 0
        args.beam_size = 10 if args.dataset_name in ['uspto_50k', '50k_class', "debug_uspto_50k", "pistachio", "uspto_full_ms"] else 30
        if args.dataset_name in ['uspto_50k', '50k_class', "debug_uspto_50k", "pistachio"]:
            args.T = 1.6
        elif args.dataset_name in ['uspto_mit', 'uspto_diverse', "uspto_full_ms"]:
            args.T = 1.2       # default 1.0
        elif args.dataset_name in ['uspto_full', 'pistachio_class']:
            args.T = 0.7
        ...
        args.beam_module = args.beam_strategy

        # print(f"args.beam_module = {args.beam_module}")
        # assert 1 == 2+66

        if args.dataset_name == "uspto_full_ms":
            args.ckpt_list = ['swa2']       # default: swa6
