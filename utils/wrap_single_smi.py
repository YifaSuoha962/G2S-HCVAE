...
from .rxn_feats import smi2graph, smi2graph_direct
from .data_loader import graph_batch, G2SBatchData
...
class InferBatch(metaclass=ABCMeta):
    ...
    def preprocess(self, smi, rxn_type=None):
        ...
        graphs = [smi2graph_direct(smi=cano_smi)]
...
class InferBatch_wo_rxns(metaclass=ABCMeta):

    def __init__(self, preprocess_dir, args):
        super(InferBatch_wo_rxns, self).__init__()
    ...
    def get_vocab_info(self):
        ...
        load vocab data for preprocess.
        """
        vocab_path = os.path.join(self.vocab_dir, f'{self.args.split_type}_vocab_dict.npz')      # 解析token并转换成smi-graph时遇到报错，有没有可能是vocab用的不对
        vocab_dict = np.load(vocab_path, allow_pickle=True)
        token_index = vocab_dict['vocab_dict']
        return {'token_idx': token_index.tolist()}
    ...
    def pack_batch(self, graph, graph_len, graph_max_len, rxn_types):
        ...
        batch_data = G2SBatchData(
            ...
            reaction_type=rxn_types,
            bi_label=bi_label,
            data_indice=torch.tensor([0]),
            pres=torch.tensor([0]),
            posts=torch.tensor([0])
        )
    ...
    def post_process(self, predict_result: torch.Tensor, predict_scores: torch.Tensor):
        ...
        # TODO: 从这里就可以拿到 top-10 SMILES

        return res_smis, None
