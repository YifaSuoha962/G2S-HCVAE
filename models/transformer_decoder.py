...
from .module_utils import RelTransformerBase, RelMultiHeadAttention, GLU, RMSNorm, PosWiseFFN, Latent_RelMultiHeadAttention
...
"""
Vatiational Transformer Decoder that incorporates layer-wise latent variables 
"""
# class RelTransformer_Decoder(RelTransformerBase):


class  lv_RelTransformer_Decoder_Block(nn.Module):
    def __init__(
        self, d_model, d_ff, h, u0, v0, u1, v1,
        sin_enc=None,
        pe='abs',
        ...
        **kwargs
        ...
        super(lv_RelTransformer_Decoder_Block, self).__init__(**kwargs)
        self.block_id = block_id
        self.alpha = alpha
        self.init_beta = init_beta
        self.cls_len = cls_len
        ...
        self._build_model(
            ...
            dropout=dropout,
            lat_sz=lat_sz,
            latent_lmf_rank=latent_lmf_rank,
            lat_fuse_type=lat_fuse_type,
        )
        self._build_cache()
    ...
    def _build_model(
        ...
        self.context_attention = Latent_RelMultiHeadAttention(
            d_model=d_model,
            h=h,
            u=u1,
            v=v1,
...
class Lv_Transformer_Decoder(RelTransformerBase):
    def __init__(
            self, d_model, d_ff, h, layer,
            pe='abs',
            sin_enc=None,
        ...
        super(Lv_Transformer_Decoder, self).__init__(
            d_rel0=d_model,
            d_rel1=d_model,
            attn0_rel_dict=attn0_rel_dict,
            attn1_rel_dict=attn1_rel_dict,
    ...
    def _build_model(
        ...
        for i in range(layer):
            self.block.add_module(
                'block' + str(i),
                lv_RelTransformer_Decoder_Block(
                    d_model=d_model,
                    d_ff=d_ff,
                    h=h,
                    u0=self.attn0_u,
    ...
    def forward(
        ...
        # for i, block in enumerate(self.block):
            # get latent variables in ith layer
            cur_lv = all_lv[i]
            tgt = self.block[i](
                x=tgt,
                context_len=context_len,
...
class Shallow_Lv_Transformer_Decoder(RelTransformerBase):
    def __init__(
            self, d_model, d_ff, h, layer,
            pe='abs',
            sin_enc=None,
        ...
        super(Shallow_Lv_Transformer_Decoder, self).__init__(
            d_rel0=d_model,
            d_rel1=d_model,
            attn0_rel_dict=attn0_rel_dict,
            attn1_rel_dict=attn1_rel_dict,
