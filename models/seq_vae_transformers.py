...
class Seq_Transformer_Builder(nn.Module):
    ...
    def encode_ref(self, data: S2SBatchData, tgt_seq: torch.Tensor):
    ...
    def forward(
        ...
        tgt_emb = self.encode_ref(data, tgt_seq)

        # TODO: 这里可以嵌入 CVAE
        Kld_loss = 0
        y_sample, Kld_loss = self.cvae(enc_mem, tgt_emb)
    ...
    def gen_predict(
        ...
        # y_sample = y_sample.argmax(-1).long()
        # beam_y_sample.append(y_sample)
        # beam_y_sample = torch.tensor(beam_y_sample).to(enc_mem.device)
        # beam_y_sample = torch.stack(beam_y_sample, dim=0)
        # beam_y_sample = beam_y_sample.view(-1, beam_y_sample.size(-1))
        # print(f"beam_y_sample.shape = {beam_y_sample.shape}")
        # assert 1 == 2
        ...
        return predict_seq, predict_score  # , y_sample, prob_per_token
    ...
    def encode_pairs(self, data: S2SBatchData):
        """
        ...
        mean_src = torch.mean(enc_mem, dim=1)
