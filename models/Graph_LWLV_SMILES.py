...
class Graph_lv_Transformer_Base():
    def __init__(
            self,
            f_vocab: int,
            f_atom: int,
        ...
        self.model = Graph_lv_Transformer_Builder(
            f_vocab=f_vocab,
            f_atom=f_atom,
            f_bond=f_bond,
            token_idx=token_idx,
...
class Graph_lv_Transformer_Builder(nn.Module):
    def __init__(
            self,
            f_vocab: int,
            f_atom: int,
        ...
        super(Graph_lv_Transformer_Builder, self).__init__()
        ...
        self.lat_fuse_type = args.lat_fuse_type
    ...
    def forward(
        ...
        enc_mem, kl_loss, post_lv, mu, sigma, log_prior, log_post, _ = self.lv_transformer_encoder(
            src=node_emb,
        ...
        kl_disc = 0
        return main_loss, token_acc, seq_acc, true_token_count, all_token_count, sub_loss, kl_loss, kl_disc
    ...
    def gen_predict(
        ...
        # print(f"node_emb.size(0) = {node_emb.size(0)}")
        # print(f"data.size(0) = {data.size(0)}")

        tmp_all_lv = []
        for i in range(beam_size):
            enc_mem, all_lv, _ = self.lv_transformer_encoder.get_prior(
                src=node_emb,
                src_len=[data.src_graph_len],
                dist=data.src_dist,
                deg=deg_emb,
