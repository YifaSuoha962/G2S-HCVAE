...
from .hugging_face_beamsearch import BeamSearchScorer, Lv_BeamSearchScorer
...
class Beam_Generate(BeamGenerateBase):
    def __init__(
        ...
        # last state of each sample
        self.last_alive = self.unfinish_batch.tolist()
        # generated ids ? [beam_size, beam_size]
...
class Lv_Beam_Generate(BeamGenerateBase):
    def __init__(
            self,
            beam_size: int,
            batch_size: int,
        ...
        super(Lv_Beam_Generate, self).__init__(
            beam_size=beam_size,
            batch_size=batch_size,
            bos_token_ids=bos_token_ids,
            pad_token_ids=pad_token_ids,
        ...
        assert self.beam_size % self.beam_group == 0
        self.beam_per_group = self.beam_size // self.beam_group
        self.beam_search = Lv_BeamSearchScorer(
            batch_size=self.batch_size,
            num_beams=self.beam_size,
            device=self.device,
            length_penalty=self.length_penalty,
        ...
        # NEW: Initialize the tracker for original beam_rxn_label indices
        # This will be set in _prepare, after beam_rxn_label is computed
        self.tracked_original_flat_indices = None
    ...
    def mem_ids(self) -> torch.Tensor:
        if self.remove_finish_batch:
            ...
            if sum(self.last_alive) < self.batch_size:
                self.ids_table[self.last_alive] = torch.arange(sum(self.last_alive) * self.beam_size,
                                                               dtype=torch.long, device=self.device).reshape(-1,
                                                                                                             self.beam_size)
                mem_ids = self.ids_table.reshape(-1).index_select(dim=0, index=mem_ids)
            self.last_alive = self.unfinish_batch.tolist()
        else:
    ...
    def _prepare(
        ...
        self.scores = np.array([], dtype=np.float32)
        self.beam_scores = torch.full((self.batch_size, self.beam_size), -float('inf'), dtype=torch.float,
                                      device=self.device)
        self.beam_scores[:, ::self.beam_per_group] = 0.
        self.beam_scores = self.beam_scores.view((self.batch_size * self.beam_size,))
        self.cur_token = torch.full((self.batch_size * self.beam_size,), self.bos_token_ids, dtype=torch.long,
                                    device=self.device)
        self.all_token = self.cur_token.clone().reshape(-1, 1)
        ...
        # This tracker will hold the initial flat index (0 to batch_size * beam_size - 1)
        # for each beam, and will be updated along with beam_idx.
        self.tracked_original_flat_indices = torch.arange(self.batch_size * self.beam_size, dtype=torch.long,
                                                          device=self.device)
    ...
    def generate(
        ...
        next_token_logits = self._scores_process(next_token_logits)
        next_token_logits = next_token_logits + self.beam_scores[:, None].expand_as(next_token_logits)
        next_token_logits = self._sample_process(next_token_logits, min_keep=self.beam_size) if self.all_token.size(
            -1) == 1 \
            else self._sample_process(next_token_logits)
        ...
        for group_id in range(self.beam_group):
            ...
            # TODO: check added part
            # NEW: Track original indices for the current group
            group_original_indices = self.tracked_original_flat_indices[group_idx]
            ...
            # Let's compute parent_original_indices based on next_ids
            # next_ids indicates which of the `group_token`s (i.e. parent beams) generated the current token.
            parent_original_indices = torch.gather(group_original_indices.reshape(self.batch_size, self.beam_per_group),
                                                   dim=-1, index=next_ids)
            parent_original_indices = parent_original_indices.view(-1)  # Flatten for process

            beam_output = self.beam_search.process(
                input_ids=group_token,
                next_scores=next_token_scores,
                next_tokens=next_tokens,
                next_indices=parent_original_indices,       # next_ids,
                pad_token_id=self.pad_token_ids,
                eos_token_id=self.eos_token_ids
            )
            self.beam_scores[group_idx] = beam_output['next_beam_scores']
            ...
            group_token = torch.cat([group_token[beam_output['next_beam_indices']],
                                     beam_output['next_beam_tokens'].unsqueeze(-1)], dim=-1)
            self.cur_token[group_idx] = group_token[:, -1]
            self.beam_idx[group_idx] = torch.div(beam_output['next_beam_indices'], self.beam_per_group,
                                                 rounding_mode='floor') * self.beam_size + \
                                       group_id * self.beam_per_group + \
                                       torch.fmod(beam_output['next_beam_indices'], self.beam_per_group)

            self.tracked_original_flat_indices[group_idx] = \
                group_original_indices.reshape(self.batch_size, self.beam_per_group)[
                    beam_output['next_beam_indices'].reshape(self.batch_size, self.beam_per_group)
                ].reshape(-1)

            self.tracked_original_flat_indices[group_idx] = torch.index_select(
                group_original_indices, dim=0, index=beam_output['next_beam_indices']
            )

        self.all_token = torch.cat([self.all_token, self.cur_token.unsqueeze(-1)], dim=-1)


    def finish_generate(self):  # Add an argument to pass the original label
        beam_output = self.beam_search.finalize(
            input_ids=self.all_token,
            final_beam_scores=self.beam_scores,
            final_beam_tokens=None,
        ...
        # Now, beam_output includes 'original_flat_indices' thanks to the mock BeamSearchScorer
        final_original_flat_indices = beam_output['original_flat_indices']  # This is the crucial part

        # Let's make a combined tensor for sorting
        # This represents all current beam paths (including finished ones if not removed)
        all_beams_data = []  # List of tuples: (score, sequence, original_flat_index)
        for i in range(self.batch_size * self.beam_size):
            # Apply length penalty to score
            # A more robust solution would be to apply length penalty in _beam_hyp.add()
            score = self.beam_scores[i].item()  # Already length-penalized by BeamSearchScorer if it did its job
            sequence = self.all_token[i]
            original_idx = self.tracked_original_flat_indices[i].item()

            # Apply the length penalty if not already applied
            current_length = (sequence != self.pad_token_ids).sum().item()
            # If length_penalty != 1.0, adjust score here for fair comparison
            # score = score / (current_length ** self.length_penalty) # This penalty is usually in BeamHypotheses

            all_beams_data.append((score, sequence, original_idx))

        seq_result = beam_output['sequences'].reshape(self.batch_size, self.beam_size, -1)[..., 1:]  # ignore BOS token
        seq_scores = beam_output['sequence_scores'].reshape(self.batch_size, self.beam_size)


        # --- Re-implementing Finalization in Beam_Generate (without using beam_output['sequences']) ---
        final_sequences_list = []
        final_scores_list = []
        final_original_indices_list = []  # This will store the final tracked indices

        # Loop through each batch
        for batch_idx in range(self.batch_size):
            # Get all beams for this batch
            batch_start_idx = batch_idx * self.beam_size
            batch_end_idx = batch_start_idx + self.beam_size

            # Get sequences, scores, and tracked original indices for this batch's beams
            batch_all_tokens = self.all_token[batch_start_idx:batch_end_idx]
            batch_beam_scores = self.beam_scores[batch_start_idx:batch_end_idx]
            batch_tracked_indices = self.tracked_original_flat_indices[batch_start_idx:batch_end_idx]

            # Collect (score, sequence, original_index) tuples
            current_batch_beams = []
            for beam_id in range(self.beam_size):
                # Ensure sequence is not just padding if it's considered finished
                sequence = batch_all_tokens[beam_id]
                score = batch_beam_scores[beam_id].item()
                original_idx = batch_tracked_indices[beam_id].item()

                # Here, add to a temporary BeamHypotheses-like structure if you want to handle EOS tokens
                # and early stopping precisely as in BeamSearchScorer.
                # Or, simply sort these tuples directly.

                current_batch_beams.append((score, sequence, original_idx))

            sorted_batch_beams = sorted(current_batch_beams, key=lambda x: x[0], reverse=True)

            # Select top `self.num_beam_hyps_to_keep` (or `self.return_num` if that's the final output count)
            # Let's use `self.return_num` as it's the final desired count
            selected_beams_for_batch = sorted_batch_beams[:self.return_num]

            for score, seq, original_idx in selected_beams_for_batch:
                final_sequences_list.append(seq)
                final_scores_list.append(score)
                final_original_indices_list.append(original_idx)

            # Pad sequences to max length
        max_seq_len = max(len(s) for s in final_sequences_list) if final_sequences_list else 1
        padded_final_sequences = torch.full((len(final_sequences_list), max_seq_len),
                                            self.pad_token_ids, dtype=torch.long, device=self.device)
        for i, seq in enumerate(final_sequences_list):
            padded_final_sequences[i, :len(seq)] = seq

        # Reshape to [batch_size, return_num, -1]
        predict_seq = padded_final_sequences.reshape(self.batch_size, self.return_num, -1)
        predict_score = torch.tensor(final_scores_list, dtype=torch.float, device=self.device).reshape(self.batch_size,
                                                                                                       self.return_num)
        predicted_original_indices = torch.tensor(final_original_indices_list, dtype=torch.long,
                                                  device=self.device).reshape(self.batch_size, self.return_num)

        # Now, use `predicted_original_indices` to get the `beam_rxn_label`
        # `beam_rxn_label` is the initial `[batch_size * beam_size]` tensor.
        # We need to pass the *original* `beam_rxn_label` to `finish_generate`.

        # Let's pass `original_beam_rxn_label` as an argument to `finish_generate`
        # and use `predicted_original_indices` to select from it.
        # Revert the function signature change for finish_generate to accept it.
        # This will simplify the previous calling code as well.

        return predict_seq, predict_score, predicted_original_indices  # Return the indices
