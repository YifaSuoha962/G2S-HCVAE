...
from .onmt_beamsearch import GNMTGlobalScorer, BeamSearch, Lv_BeamSearch
...
class Beam_Generate(BeamGenerateBase):
    def __init__(
        ...
        # To store the probabilities for each token after beam search
        self.top_k_log_probs = []
    ...
    def generate(
        ...
        # nxt_token_logits is log version
        self.beam_search.advance(next_token_logits, None)

        # Store top k log probabilities
        max_values, _ = torch.max(next_token_logits, dim=1)
        # 将 max_values 的形状调整为 [10, 1]
        max_values = max_values.unsqueeze(1)  # Get top 10 log probs
        self.top_k_log_probs.append(max_values.detach().cpu())  # Store the log probs
    ...
    def finish_generate(
        ...
        return seq_result, seq_scores, self.top_k_log_probs
...
class Lv_Beam_Generate(BeamGenerateBase):
    def __init__(
        ...
        self.beam_search = Lv_BeamSearch(
            beam_size=self.beam_size,
            batch_size=self.batch_size,
            pad=self.pad_token_ids,
            bos=self.bos_token_ids,
    ...
    def _prepare(
            ...
            src_map=None,
            target_prefix=None,
            initial_rxn_label=None      # correspond to beam_rxn_label
    ):
        _, _, _, _ = self.beam_search.initialize(
            memory_bank=memory_bank,
            src_lengths=src_lengths,
            ...
            device=self.device,
            target_prefix=target_prefix,
            initial_rxn_label=initial_rxn_label
        )
        self.step = 0
    ...
    def generate(
        ...
        if self.any_finish:
            self.beam_search.update_finished_lv_label()
        self.step += 1
    ...
    def finish_generate(
        ...
        # predictions: [beam_size, seq_len]
        predictions, scores, top_rxn_labels = self.beam_search.predictions, self.beam_search.scores, self.beam_search.top_rxn_labels
        max_result_len = 0
        for batch_res in predictions:
            for beam_res in batch_res:
                max_result_len = len(beam_res) if len(beam_res) > max_result_len \
        ...
        # Initialize for storing the selected reaction labels
        seq_rxn_labels = torch.zeros((self.batch_size, self.return_num), dtype=torch.long)  # Assuming long for labels
        ...
        for batch_id, (batch_res, batch_score) in enumerate(zip(predictions, scores)):
            for beam_id, (beam_res, beam_score) in enumerate(zip(batch_res, batch_score)):
                ...
                beam_res = beam_res.detach().cpu()
                pad_beam_res = torch.cat([beam_res, torch.tensor([self.pad_token_ids] * \
                                                                 (max_result_len - len(beam_res)), dtype=torch.long)],
                                         dim=0)
                seq_result[batch_id, beam_id] = pad_beam_res
                seq_scores[batch_id, beam_id] = beam_score

                # Retrieve the corresponding rxn_label for this final beam
                # The `top_rxn_labels` from BeamSearch should directly correspond
                # to the `predictions` and `scores` order.
                seq_rxn_labels[batch_id, beam_id] = top_rxn_labels[batch_id, beam_id]

        return seq_result, seq_scores, self.top_k_log_probs, seq_rxn_labels
