...
class BeamSearchBase(DecodeStrategy):
    ...
    def initialize_(self, memory_bank, memory_lengths, src_map, device,
        ...
        # shape below: [batch_size, beam_size]
    ...
    def advance(self, log_probs, attn):
        ...
        # Batch_Size: _B = log_probs.shape[0] // self.beam_size
...
########### Newly added modules ############
class Lv_BeamSearch(BeamSearchBase):
    """
            Beam search for seq2seq/encoder-decoder models with latent variables
    """

    def initialize(self, memory_bank, src_lengths, src_map=None, device=None,
                   target_prefix=None, initial_rxn_label=None):  # Add initial_rxn_label here
        """Initialize for decoding.
        Repeat src objects `beam_size` times.
        """
        ...
        super(Lv_BeamSearch, self).initialize_(
            memory_bank, self.memory_lengths, src_map, device, target_prefix)

        # Store and tile init_rxn_label
        if initial_rxn_label is not None:
            self.initial_rxn_label_for_beams = initial_rxn_label
            self.original_batch_size = memory_bank.size(1)  # Store original batch size for later reshaping/indexing
        else:
            self.initial_rxn_label_for_beams = None
            self.original_batch_size = memory_bank.size(1)  # TODO: check val = batch_size

        # Initialize attributes to store the final rxn labels
        # self.top_rxn_labels = torch.zeros((self.batch_size, self.n_best), dtype=torch.long) # This will hold the rxn labels for the N-best beams

        # self.memory_lengths: [batch_size * beam_size]
        return fn_map_state, memory_bank, self.memory_lengths, src_map

    # def advance_lv(self, log_probs, attn):
    #     self.advance(log_probs, attn)
    #
    #     pass

    def update_finished_lv_label(self):
        self.update_finished()
        if not hasattr(self, '_final_beam_origin_indices') or self._final_beam_origin_indices is None:
            self._final_beam_origin_indices = torch.zeros((self.batch_size, self.n_best), dtype=torch.long,
                                                          device=self.initial_rxn_label_for_beams.device)
            for b_idx in range(self.batch_size):
                # This is a simplified mapping: assume the n_best results come from the first n_best initial beams.
                # This is NOT generally true for a real beam search, but serves as a placeholder for the *structure*.
                self._final_beam_origin_indices[b_idx, :] = torch.arange(self.n_best,
                                                                         device=self.initial_rxn_label_for_beams.device) + b_idx * self.beam_size
        if self.initial_rxn_label_for_beams is not None:
            # Populate self.top_rxn_labels using the extracted indices
            # Ensure _final_beam_origin_indices is correctly populated by your BeamSearch's internal logic
            # to reflect the actual parent beam indices of the final n_best sequences.
            self.top_rxn_labels = self.initial_rxn_label_for_beams[self._final_beam_origin_indices]
        else:
            # If no initial_rxn_label was provided, set top_rxn_labels to None or zeros
            self.top_rxn_labels = torch.zeros((self.batch_size, self.n_best), dtype=torch.long,
                                              device=self.predictions.device)  # Or fill with a special value like -1
