    ...
    for ckpt_name in args.ckpt_list:
        ...
        with (torch.no_grad()):
            ...
            for step, batch in zip(teval, data_loader):
                ...
                predict_result, predict_scores, predict_rxn_types = module.model_predict(
                    data=batch,
                    args=args,
                    beam_size=args.beam_size
                )
                ...
                # transform to smi
                if args.train_task in ['prod2subs', 'bidirection']:
                ...
                beam_acc, beam_invalid, beam_smis, tgt_smis, pred_matching, pred_invalid, rxns_coverage = beam_result_process_1_to_N(
                ...
                predict_rxn_types_list = predict_rxn_types.tolist()
                # final_rxn_types.append(predict_rxn_types_list)
                concat_rxn_types.extend(predict_rxn_types_list)
                ...
                # collect_pred_react.extend(column_pred_smi)
                # collect_pred_match.extend(column_pred_match)
        ...
        for key in retro_result_dict.keys():
            ...
            res_rxn_types.append(','.join([str(x) for x in retro_result_dict[key]['rxn_type']]))

        # TODO:
        # 这句话把dict文件转成pandas数据表，其中key是列标题，列中的所有值都是value中的元素
        retro_res = pd.DataFrame({'products': res_prods, 'reactants': res_reacts, 'predictions': res_preds, 'rxn_types': res_rxn_types})

        # 把 product 复制成prediction的个数（个数就是，分割的数量），存成一列列表:len = num_prod * beam_size
        # 把 prediction 展开，顺序存成一列，长度和product相同
        # 针对每个prediction，去查询retro_res中product对应的reactants,根这些查到的reactants做字符串匹配
        # 匹配做法（prediction[i] = any of reactants）[找到1个就是1，都找不到为0]
        # 把每个prediction的匹配结果存成一列，长度和扩展的prediction以及products相同
        # 将三个列表变成pandas数据表的形式，保存成csv文件
