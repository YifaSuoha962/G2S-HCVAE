...
from transform_csv_to_json import contruct_one2many_map
import json
...
def add_result_args(parser):
    ...
    group.add_argument("--data_name", help="Dataset name",
                       choices=["uspto_50k", "uspto_diverse", "uspto_full_ms"], type=str,
                       default="uspto_50k")
    ...
    # mol_transformer 路径
    parser.add_argument("--mol_trans_dir", default="../MolecularTransformer/")
    parser.add_argument("--fwd_batch_size", type=int, default=256)
    # 使用json
    parser.add_argument("--not_use_json", action="store_true")
...
def eval_func(df, top_k=[1]):
    ...
    return pd.DataFrame(results).T, df
...
def collecet_retro_res(result_file, args):
    ...
    rxn_types_df = []     # inferred reaction type
    ranks_inside_df = []

    original_df = pd.read_csv(result_file)

    if args.data_name in ['uspto_diverse']:     # 'uspto_50k',
        ans_path = os.path.join(f"data/{args.data_name}", "raw_test.json")
        with open(ans_path, 'r') as jfile:
            ans = json.load(jfile)
    elif not args.not_use_json:
        products_list = original_df['products'].to_list()
        reactants_list = original_df['reactants'].to_list()
        ans = contruct_one2many_map(src=products_list, tgt=reactants_list, rxn_prod_dict={}, phase='test',
                                    remove_atom_number=True, trim=False)
        with open("test_on2_many.json", 'w') as jfile:
            json.dump(ans, jfile, indent=4)

    accuracies = np.zeros([original_df.shape[0], top_k], dtype=np.float32)
    validities = np.ones([original_df.shape[0], top_k], dtype=np.float32)
    ...
    for i in range(original_df.shape[0]):
        ...
        cleaned_rxn_types = original_df['rxn_types'][i].replace('"', '')
        pred_rxn_types = cleaned_rxn_types.split(',')

        if original_df['products'][i] != '':
            cano_prod = original_df['products'][i]
        else:
            cano_prod = canonicalize_smiles(original_df['products'][i], trim=False, stereo=True)
        ...
        for k, pred in enumerate(pred_reacts):
            ...
            if not cano_pred:
                invalid = 1
                validities[i, k] = 0.0
            if args.not_use_json:
                if cano_pred == cano_react:
                    is_correct = 1
                    accuracies[i, k:] = 1.0
            else:
                if cano_pred in ans[cano_prod]['reactant']:
                    is_correct = 1
                    accuracies[i, k:] = 1.0
                    if 'cover' in ans[cano_prod].keys():
                        ans[cano_prod]['cover'] += 1
                    else:
                        ans[cano_prod]['cover'] = 1
                    ans[cano_prod]['reactant'].remove(cano_pred)

            products_df.append(cano_prod)
            true_reactants_df.append(canonicalize_smiles(cano_react, trim=False, stereo=True))
            if cano_prod != '':
                pred_reactants_df.append(canonicalize_smiles(cano_pred, trim=False, stereo=True))
            else:
                pred_reactants_df.append(cano_pred)
            correct_tags_df.append(is_correct)
            invalid_tags_df.append(invalid)
            rxn_types_df.append(pred_rxn_types[k])
            ranks_inside_df.append(k+1)

    retro_res = {'product': products_df, 'pred_reactant': pred_reactants_df,
                 'true_reactant': true_reactants_df, 'exact_match': correct_tags_df,
                 'invalid': invalid_tags_df, 'rxn_type': rxn_types_df, 'rank':ranks_inside_df}
    ...
    # cal validity
    sum_validity = np.sum(validities, axis=0)
    cum_validity = np.cumsum(sum_validity, axis=0)

    # cal coverage rate
    if args.data_name != 'uspto_50k':
        metric = {}
        for k in ans.keys():
        ...
        for k in metric.keys():
            ...
            print(f"for 1-to-{k} reaction, coverage = {metric[k]['coverage']}")

    # print(f"sum_accuracies.shape[0] = {sum_accuracies.shape[0]}")
    # print(f"num_rxns = {original_df.shape[0]}")
    for n in range(top_k):
        print(f"Top {n + 1} exact match accuracy: {sum_accuracies[n] / original_df.shape[0] * 100: .2f} %")
        print(f"Top {n + 1} validity: {cum_validity[n] / (original_df.shape[0] * (n + 1)) * 100: .2f} %")
...
def round_trip_exp(parser: argparse.ArgumentParser, args, df):
    ...
    tic = time.time()
    print(f"batch_size = {args.fwd_batch_size}")
    # assert 1 == 2
    ...
    scores, pred_products = translator.translate(
        ...
        batch_size=args.fwd_batch_size,
...
if __name__ == "__main__":
    ...
    if not args.load_rt_res:
        if not args.load_inter:
            ...
            retro_df.to_csv(f"{args.result_file.split('.')[0]}-tmp-res.csv", index=False)
    ...
    if args.top_k ==10:
        ...
        top_k_list = [1]
    round_trip_metric, round_trip_info = eval_func(round_trip_df, top_k_list)

    target_file = os.path.join(data_dir, f"{args.result_file.split('/')[-1]}")
    round_trip_info.to_csv(target_file, index=False)
    round_trip_metric.to_csv(f"{target_file.split('.')[0]}-scores.csv", index=False)
