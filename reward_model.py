...
def get_products_mask(x):
    ...
    mask = torch.matmul(y, rank)        # y-> [bz, max_len, max_len]
    # matmul 在最后两维上操作
    # mask: [bz, max_len, max_len]
...
def get_position_embedding_reward(x, embedding_size):
...
def get_mutual_mask_reward(x):
    ...
    mask = torch.matmul(y, rank)        # [n_sample, len_input, len_output]
    return mask.permute(0,2,1)          # [n_sample, len_output, len_input]


def get_output_mask_reward(x):
...
class RewardTransformerConfig(object):
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids):
        position_embeddings = get_position_embedding_reward(input_ids, self.embedding_size)
...
class RewardTransformer(nn.Module):
    ...
    def forward(self, input_ids, output_ids, input_mask, output_mask, mutual_mask):
        ...
        input_mask = get_attention_mask_reward(input_mask, self.num_attention_heads)
        output_mask = get_attention_mask_reward(output_mask, self.num_attention_heads)
        mutual_mask = get_attention_mask_reward(mutual_mask, self.num_attention_heads)
